...
from experiments.rnn_common import LSTMExtractor, predict_logits, train_model
...
class LSTMSiameseModelV1(nn.Module):
    def __init__(self, embeddings, hidden_units, dropout=0.2):
        super(LSTMSiameseModelV1, self).__init__()
        self.extractor = LSTMExtractor(embeddings, hidden_units)

        self.classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(8 * hidden_units, 2 * hidden_units),
    ...
    def forward(self, q1_token_ids, q2_token_ids):
        q1_features = self.extractor(q1_token_ids)
        q2_features = self.extractor(q2_token_ids)
        features = torch.cat(
            [torch.abs(q1_features - q2_features), q1_features * q2_features],
            1,
        )
...
def train_1fold(output_dir, fold_id, config, num_workers, device, dryrun):
    ...
    model = LSTMSiameseModelV1(
        embedding_matrix, hidden_units=config["hidden_units"]
    ).to(device)
