...
class TransformerModelV3(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(
                self.encoder.config.hidden_size * 4 + feature_dim, hidden_units
            ),
    ...
    def forward(
        ...
        bert_features = torch.cat(bert_output.hidden_states[-4:], 2)[:, 0, :]
        return self.classifier(torch.cat((bert_features, features), 1))
